  ...
  return (
    <div className="page">
      <div className="container">
        <Link to={`/championships/${driver.championship_id}`} className="btn btn-ghost btn-sm mb-4" style={{ display: 'inline-flex' }}>
          ← Back to series
        </Link>

        <div className="card fade-in" style={{ marginBottom: 24, padding: 28 }}>
          <div className="flex items-center gap-6 flex-wrap">
            <div className="avatar avatar-xl" style={{ background: 'var(--accent-light)', color: 'var(--accent)', fontSize: '1.5rem' }}>
              {driver.avatar ? <img src={driver.avatar} alt="" style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover' }} /> : driver.name?.substring(0, 2).toUpperCase()}
            </div>
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              <p className="text-secondary">
                <span className="team-dot" style={{ background: driver.team_color }} /> {driver.team_name || 'No team'} · {driver.championship_name} ({driver.season})
              </p>
            </div>
          </div>
        </div>
            ...
            <div style={{ width: '100%', height: 280 }}>
              <ResponsiveContainer>
                <LineChart data={chartData}>
                  ...
                  <Line type="monotone" dataKey="Points" stroke="var(--accent)" strokeWidth={2.5} dot={{ r: 4, fill: 'var(--accent)' }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="table-wrapper" style={{ border: 'none', borderRadius: 0 }}>
              <table>
                ...
                <tbody>
                  {results.map((r: any) => (
                    <tr key={r.id}>
                      <td><Link to={`/races/${r.race_id}`} style={{ color: 'var(--text)', fontWeight: 500 }}>{r.race_name}</Link></td>
                      <td className="text-secondary">{r.circuit}</td>
                      <td className="text-muted text-sm">{new Date(r.date).toLocaleDateString()}</td>
      ...
      <style>{`
        .driver-name { font-size: 1.5rem; font-weight: 700; letter-spacing: -0.02em; }
